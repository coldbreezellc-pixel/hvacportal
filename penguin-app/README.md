...
| Home | Landing screen with tiles for every tool, open-work-order, low-stock and PM follow-up counts. Time Off (not moved yet) opens the old Railway portal in a new tab. |
| PM Sheet | The full preventive-maintenance sheet from the old portal: PPE gate, safety checklist with LOTO photos and per-tech safety signatures, per-equipment checklists with amp readings and findings, photos, post-job checklist, completion signatures. Autosaves a draft on the phone; on **Email Report** it builds the PDF on the phone, uploads photos/signatures/PDF to the `pm-files` bucket, saves the record and emails it via Resend. Works offline — the upload + email happen when service returns. |
| PM Records | Month-grouped archive of every completed PM with search and a follow-up filter; open one to print it, download the PDF, resend the email, or (admin) delete it. Admins can import the PMs archived by the old portal with one tap (`legacy/pm-records`, photos moved to storage, idempotent). |
| Slack intake | Slack "Facilities Help Request" posts in `#help-facilities` become work orders — Englewood Cliffs only, HVAC / plumbing request types only, de-duplicated by Slack message timestamp, `source = slack`. Three ways in, all sharing `lib/slack-intake.ts`: the app reads the channel itself with `SLACK_USER_TOKEN` (`POST /api/intake/slack/pull`, run when Work Orders opens, from the **Pull Slack** button, and hourly by a Claude routine); the routine can also hand messages in via `POST /api/intake/slack` (bearer `SLACK_INTAKE_TOKEN`); and, when the Slack app's Event Subscriptions point at `/api/slack/events`, requests arrive in real time. Rotating tokens are renewed automatically and kept in `app_settings`. |
| Work Orders → ⚡ From Slack | Paste a help request copied out of Slack and tap **Generate**: title, building (900/904), type and priority are detected and a prefilled work order opens for review. No Slack API access needed. |
