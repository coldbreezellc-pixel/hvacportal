...
| Slack intake | `POST /api/intake/slack` (bearer `SLACK_INTAKE_TOKEN`) turns Slack "Facilities Help Request" posts into work orders, de-duplicated by Slack message timestamp. An hourly Claude routine reads `#help-facilities` with the owner's own Slack login and calls it, so no Slack app or admin rights are needed. Orders carry `source = slack` and the requester's name. |
